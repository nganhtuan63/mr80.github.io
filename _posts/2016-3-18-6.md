...
This post is a summary of what I've learned about "modern" Javascript core concepts including Scope and Closure, Function, Oject Prototype, Module Export/Require.
...
In this example, we declare 2 functions: **printMessage** and **processReult()**. The processResult has 2 parameters: **result** parameter and **aCallbackFunction** parameter - which is a parameter function that we will call when we need to do something after done in the processResult. Later, we call processResult() and we input 2 params values: 'done' for result param - printMessage function for aCallBackFunction param.

Besides, we can also define the anonymous callback function without defining it first:
...
1. **Asynchronous programming:** Sometimes, in a function, it takes time to finish a task such as: Sending requests to server and waiting for response. In that case, when that task is running, other tasks have to wait. Javascript enables async programming for us to deal with this situation: While a task is running and not finished yet, we call a callback function to do other task like showing the progress.
